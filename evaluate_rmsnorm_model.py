...
"""
Evaluate Phi-3 Model with Fine-tuned RMSNorm Weights

This script loads a Phi-3 model, applies fine-tuned RMSNorm weights from LLaMA-Factory training,
and evaluates the model using lm-evaluation-harness.

Usage:
    python evaluate_rmsnorm_model.py --rmsnorm_weights_path ./saves/phi3-variance-reg-alpaca-layers-2-4/rmsnorm_weights.pt
    python evaluate_rmsnorm_model.py --rmsnorm_weights_path ./saves/phi3-variance-reg-alpaca-layers-2-4/rmsnorm_weights.pt --tasks "wikitext,arc_challenge,hellaswag"
    python evaluate_rmsnorm_model.py --rmsnorm_weights_path ./saves/phi3-variance-reg-alpaca-layers-2-4/rmsnorm_weights.pt --limit 100
...
class RMSNormModelLoader:
    ...
    @staticmethod
    def load_base_model(model_name: str = "microsoft/Phi-3-mini-4k-instruct", device: str = "auto") -> Any:
        """
        ...
        try:
            ...
            logger.info(f"Successfully loaded base model on device: {model.device}")
            return model
    ...
    def load_model_with_rmsnorm_weights(cls, rmsnorm_weights_path: str, 
        ...
        """
        # Load base model
        model = cls.load_base_model(base_model, device)
...
class RMSNormEvaluator:
    ...
    def evaluate(self, tasks: Optional[List[str]] = None, limit: Optional[int] = None) -> Dict[str, Any]:
        """
        Evaluate the RMSNorm fine-tuned model.
        
        Args:
            tasks: List of tasks to evaluate (None for default)
...
def setup_argument_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate Phi-3 Model with Fine-tuned RMSNorm Weights",
...
def main():
    ...
    try:
        ...
        if "results" in results and results["results"].get("success", False):
